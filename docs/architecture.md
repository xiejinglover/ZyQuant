...
缓存逻辑位于项目内的 `.zyquant/cache/factors`，相对明确的
`project_root` 解析。生产因子先在独立的 `prepare` 阶段原子发布，
再由 `verify` 和回测以 `require` 模式只读消费。权威快照的标准
cutoff 为 `2026-07-24`；回测结束日可更早，不改变缓存身份。
