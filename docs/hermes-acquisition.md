...
Hermes 的融资偿还额和融券偿还量是有符号流量：极少数负值是上游用于维持
余额恒等式的历史冲正，不是缺失值。canonical 原样保留这两个字段的负冲正；
融资/融券余额、买入额和卖出量仍执行非负约束。
...
对改代码但未更换公司身份的证券，若旧代码记录没有独立行情、同一
`PARTY_ID` 只有一个存续行情身份，则旧代码会作为审计别名映射到该身份。
例如 `000043` 的日内因子映射到 `001914.XSHE`；别名映射行数进入质量清单。
